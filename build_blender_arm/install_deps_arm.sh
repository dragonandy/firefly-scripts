...
compile_OCIO() {
  ...
  if [ ! -d $_inst ]; then
    ...
    if uname -m | grep -q '86'; then
      cmake_d="$cmake_d -D OCIO_USE_SSE=ON"
    else
      cmake_d="$cmake_d -D OCIO_USE_SSE=OFF"
    fi

    if file /bin/cp | grep 'ARM' | grep -q '32-bit'; then
      cflags="-fPIC -march=armv7"
    elif file /bin/cp | grep 'ARM' | grep -q '64-bit'; then
      cflags="-fPIC -march=armv8"
    elif file /bin/cp | grep 'Intel' | grep -q '32-bit'; then
...
compile_OIIO() {
  ...
  if [ ! -d $_inst ]; then
    ...
    if uname -m | grep -q '86'; then
      ...
      cmake_d="$cmake_d -D USE_SIMD="fpu=neon""
    else
      cmake_d="$cmake_d -D USE_SIMD="""
    fi
